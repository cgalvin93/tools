...
Each of these strings is followed by 4 (optionally 5 ) columns of numbers:
x0, xtol, k, covalent/periodicity, and number of samples.
...
The 3rd column specifies the force constant k, or the strength of this particular parameter.
If x is the value of the constrained parameter, the score penalty applied will be:
 0 if |x - x0| < xtol and k * ( |x - x0| - xtol ) otherwise
This 3rd column is only relevant for enzdes, and the number in it is not used by the matcher.
The 4th column has a special meaning in case of the distanceAB parameter. It specifies whether
the constrained interaction is covalent or not. 1 means covalent, 0 means non-covalent.
If the constraint is specified as covalent, Rosetta will not evaluate the vdW term between Res1:Atom1 and Res2:Atom1 and their [1,3] neighbors.
For the other 5 parameters, the 4th column specifies the periodicity per of the constraint.
The 5th column is optional and specifies how many samples the matcher,
if using the classic matching algorithm ( see the matcher documentation ),
 will place between the x0 and x0 +- tol value.


example #lysine-terminal carbonyl oxygen
CST::BEGIN
  TEMPLATE::   ATOM_MAP: 1 atom_name: O1 C1 C2
  TEMPLATE::   ATOM_MAP: 1 residue3: cit

  TEMPLATE::   ATOM_MAP: 2 atom_name: NZ CE CD
  TEMPLATE::   ATOM_MAP: 2 residue1: K

  CONSTRAINT:: distanceAB:    2.90   0.30 100.00  1
  CONSTRAINT::    angle_A:  124.6   6.00 100.00  360.00
  CONSTRAINT::    angle_B:  142.0   5.00  50.00  360.00
  CONSTRAINT::  torsion_A:  77.3 10.00  50.00  360.00
  CONSTRAINT::  torsion_B:  162.2  10.00  25.00  180.00
  CONSTRAINT:: torsion_AB:  -122.7   45.00   0.00  180.00
CST::END
'''
